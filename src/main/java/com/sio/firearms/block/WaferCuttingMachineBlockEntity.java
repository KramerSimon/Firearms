...
public class WaferCuttingMachineBlockEntity extends EnergyStorageBlock implements MenuProvider {
    ...
    public WaferCuttingMachineBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.WAFER_CUTTING_MACHINE.get(), pos, state, CAPACITY, MAX_RECEIVE, 0);
    }

    public ItemStackHandler getInventory() { return inventory; }

    @Override public Component getDisplayName() { return Component.translatable("block.firearms.wafer_cutting_machine"); }

    @Override public AbstractContainerMenu createMenu(int id, Inventory inv, Player player) {
        return new WaferCuttingMachineMenu(id, inv, inventory, data);
    }
    ...
    public void serverTick() {
        ...
        ItemStack result = new ItemStack(ModItems.SILICON_WAFER.get(), 4);

        boolean hasBlade = !blade.isEmpty() && stackIs(blade, "firearms:diamond_saw_blade")
                && blade.getDamageValue() < blade.getMaxDamage();
        boolean canProcess = stackIs(in0, "firearms:silicon_ingot") && hasBlade
