...
public class EuvLithographyScreen extends AbstractContainerScreen<EuvLithographyMenu> {

    private static final ResourceLocation TEXTURE =
            ResourceLocation.fromNamespaceAndPath("firearms", "textures/gui/euv_lithography_controller.png");

    public EuvLithographyScreen(EuvLithographyMenu menu, Inventory playerInventory, Component title) {
    ...
    public void render(GuiGraphics gui, int mouseX, int mouseY, float partialTick) {
        ...
        if (maxF > 0 && menu.getFluidAmount() > 0) {
            ...
            gui.fill(leftPos + 27, topPos + 66 - h, leftPos + 39, topPos + 66, 0xFF8000FF);
        }

        if (!menu.isStructureValid()) {
            gui.drawCenteredString(font, Component.literal("Structure Invalid"),
                    leftPos + imageWidth / 2, topPos + 55, 0xFFFF0000);
        } else if (!menu.isCleanRoom()) {
            gui.drawCenteredString(font, Component.literal("Clean Room Violation"),
                    leftPos + imageWidth / 2, topPos + 55, 0xFFFFFF00);
        }
    ...
    protected void renderTooltip(GuiGraphics gui, int mx, int my) {
        ...
        if (mx >= leftPos + 27 && mx < leftPos + 39 && my >= topPos + 14 && my <= topPos + 66) {
            gui.renderTooltip(font,
                    Component.literal("Photoresist: " + menu.getFluidAmount() + " / " + menu.getFluidMax() + " mB"),
                    mx, my);
            return;
        }
